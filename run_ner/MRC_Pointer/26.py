...
def train(args, train_dataset, student_model, teacher_model, tokenizer):
    ...
    for epoch in train_iterator:
        ...
        for step, batch in enumerate(epoch_iterator):
            ...
            #         ############
            #         if fast_p.dim() == 1:
            #             # print(teacher_weights[fast_n])  # (768,)
            #             # print(fast_p.shape)  # (512,)
            #             diff = teacher_weights[fast_n].size(0) - fast_p.size(0)
            #             if diff != 0:
            #                 new_fast_p = torch.cat((fast_p, torch.zeros(diff).to(args.device)), dim=0)  # (512,)+(256,)->(768,)
            #             else:
            #                 new_fast_p = fast_p
            #             gradient = teacher_weights[fast_n] - new_fast_p.to(args.device)  # θ_t - θ_s'
            #         elif fast_p.dim() == 2:
            #             # print(teacher_weights[fast_n].shape)  # (30522, 768) | (768, 768) | (768, 768)
            #             # print(fast_p.shape)  # (30522, 512) | (512, 512) | (768, 768)
            #             diff1 = teacher_weights[fast_n].size(0) - fast_p.size(0)
            #             diff2 = teacher_weights[fast_n].size(1) - fast_p.size(1)
            ...
            #             else:
            #                 raise NotImplementedError
            #             gradient = teacher_weights[fast_n] - new_fast_p  # θ_t - θ_s'
            #         else:
            #             raise NotImplementedError()
            #         ############
            #
...
def main():
    ...
    # Required parameters
    parser.add_argument("--task_name", default="cola", type=str, required=True, help="The name of the task to train selected in the list: " + ", ".join(processors.keys()))
    ...
    #         model = model_class.from_pretrained(checkpoint)  # './model/RTE/teacher/best'
