...
crf_ID_TO_LABEL = {value: key for key, value in crf_LABEL_TO_ID.items()}
...
class BertBiLstmCrf(nn.Module):
    def __init__(self, args, model_name):
        ...
        self.bilstm = nn.LSTM(config.hidden_size, int(config.hidden_size / 2), num_layers=2, batch_first=True,
                              bidirectional=True)
...
def evaluate(args, model, optimizer ,features, current_epoch,tag="dev_or_test"):
    ...
    for batch in dataloader:
        ...
        with torch.no_grad():
            ...
            # logits = logits.view(-Data_set, args.num_class)

            tags = model.crf.decode(logits, batch['attention_mask'])
            preds += tags.cpu().numpy().flatten().tolist()

    preds, keys = list(zip(*[[pred, key] for pred, key in zip(preds, keys) if key != -1]))

    preds = [crf_ID_TO_LABEL[pred] for pred in preds]
    keys = [crf_ID_TO_LABEL[key] for key in keys] #将index转为key形式
    model.zero_grad()
...
def main(args):
    ...
    model = BertBiLstmCrf(args,args.model_name_or_path[0]) #模型
...
if __name__ == "__main__":
    from Code.run_ner.MRC_Pointer_Set_Config import set_bert_lstm_crf_config
    args_=set_bert_lstm_crf_config()
