...
class Create_BertSpan_prefix_puning_disti(nn.Module):
    def __init__(self, args):
        ...
        self.dim_of_model = 768
        self.attention_1 = MultiheadAttention(12, d_model=self.dim_of_model, dropout=0.1)
        self.LayerNorm1 = nn.LayerNorm(hidden_size)
        self.LayerNorm2 = nn.LayerNorm(hidden_size)

        # if args.self_attention:
        #     self.rnn = nn.GRU(self.dim_of_model * 2, int(self.dim_of_model / 2), num_layers=1 , bidirectional=True, batch_first=True, dropout=0.2)
        # else:
        #     self.rnn = nn.GRU(self.dim_of_model, int(self.dim_of_model / 2), num_layers=1, bidirectional=True, batch_first=True, dropout=0.2)

        if args.self_attention:
            self.rnn = nn.LSTM(self.dim_of_model * 2, int(self.dim_of_model / 2), num_layers=1 , bidirectional=True, batch_first=True,dropout=0.2)
        else:
            self.rnn = nn.LSTM(self.dim_of_model, int(self.dim_of_model / 2), num_layers=1, bidirectional=True, batch_first=True,dropout=0.2)

        self.maxpool = nn.MaxPool1d(32)
        # self.classifier = nn.Linear(self.dim_of_model, self.dim_of_model)
        # self.softmax = nn.LogSoftmax(dim=Data_set)
    ...
    def forward(self, args, input_ids, attention_mask, token_label, have_ans, ans_nums, start_positions=None, end_positions=None, ):
        ...
        if start_positions is None or end_positions is None:  #标签为空--test
            ...
            model_out_ = model_output[3]
            model_out_se_logit = model_output[1:3]

            if args.self_attention:
                # model_out_att = self.LayerNorm1(model_out_)
                # att_ = self.attention_1(model_out_att, model_out_att, model_out_att)
                att_ = self.attention_1(model_out_, model_out_, model_out_)
                att_drop = self.dropout(att_)
                # model_out_ = model_out_att + att_drop
                # model_out_ = att_drop  #只取经过attention 权值
                model_out_ = torch.cat([model_out_, att_drop],dim=2)
                #pool
                # model_out_ = F.avg_pool1d(model_out_att.transpose(2, Data_set), kernel_size=50).squeeze()
                # model_out_ = F.max_pool1d(x.transpose(2, Data_set), kernel_size=50).squeeze()
            ...
            ##################### decode #####################
            rnn_output, _ = self.rnn(model_out_)
            # lstm_output = nn.ReLU()(lstm_output)

            sequence_output = rnn_output
            # sequence_output = model_out_ #不接attention 和 decode
            sequence_output = self.dropout(sequence_output)
            ...
            if args_.fit_type == "reg":
                ...
                # if args.have_ans_predict:
                #     loss_out = decode_loss + loss + args_.alpha_t * reg_loss + loss_have_ans # 联合训练的总损失
                # else:
                #     loss_out = decode_loss + loss + args_.alpha_t * reg_loss # 联合训练的总损失

                if args.have_ans_predict:
                    loss_out = loss + args_.alpha_t * reg_loss + loss_have_ans # 联合训练的总损失
                else:
                    loss_out = loss + args_.alpha_t * reg_loss # 联合训练的总损失

                # model_output = (loss_out,) + model_out
                model_output = (loss_out,) + (model_out_se_logit)
...
def train(args, model, train_features, benchmarks):
    train_dataloader = DataLoader(train_features, batch_size=args.batch_size, shuffle=True, collate_fn=collate_fn_span_have_ans, drop_last=True)
...
def main(args):
    ...
    model = Create_BertSpan_prefix_puning_disti(args) #模型
...
if __name__ == "__main__":
    from Code.run_ner.MRC_Pointer_Set_Config import set_bert_span_prefix_tuning_regularization_have_ans
    args_=set_bert_span_prefix_tuning_regularization_have_ans()
    ...
    path="have_ans_predict_"+str(args_.have_ans_predict)+"_"+"attention_"+str(args_.self_attention)+"_"+args_.loss_type+"_"+args_.model_dataset+"_"+str(args_.seed)+"_"+str(args_.prefix_seq_len)
