...
class BertSpan_student(nn.Module):
    ...
    def forward(self, args, input_ids, attention_mask, token_label, start_positions=None, end_positions=None):
        ...
        out_logits = (start_logits, end_logits,)
        ...
        if start_positions is not None and end_positions is not None:
            ...
            outputs = (total_loss,) + out_logits
        return outputs  # loss , (start_logits, end_logits,) , outputs[2:]

class BertSpan_teacher(nn.Module):
    def __init__(self, args, model_name):
        super().__init__()
        self.args = args
        self.model_name_ = model_name
...
def distance_cal(args, output_stu, output_ter,input_ids, attention_mask, token_label, start_positions=None, end_positions=None):

            loss_stu=output_stu[0]
            # model_output = outputs[0]  # 返回学生 model的预测
            start_logits = []
            start_logits.append(output_stu[1])
            start_logits.append(output_ter[0])
            end_logits = []
            end_logits.append(output_stu[2])
            end_logits.append(output_ter[1])
            ...
            if args.fit_type == "reg":
                ...
                token_input_label = (token_label.view(-1) != -1).to(start_logits_[1])

                start_reg_loss = sum([distance_(start_logits_[0], start_logits_[1], args) * token_input_label])
                end_reg_loss = sum(
                    [distance_(end_logits_[0], end_logits_[1], args) * token_input_label])
                ...
                alpha=0.7
                loss_out = alpha*loss_stu + (1-alpha)* reg_loss  # 联合训练的总损失
            # model_output = (loss_out,) + model_output
            model_output = (loss_out,)
            return model_output


def train(args, model_s, model_t, train_features, benchmarks):
    ...
    if os.path.exists(output_model):
        ...
        model_s.load_state_dict(torch.load(output_model))
    else:
        print("\n==================start_training===================")

    optimizer = Adam(model_s.parameters(), lr=args.learning_rate, eps=args.eps)
    ...
    for epoch in range(int(args.num_train_epochs)):
        ...
        for step, batch in enumerate(tqdm(train_dataloader)):
            ...
            # with autocast():
            outputs_stu = model_s(args, **inputs)  # 将一批次数据输入模型中，得到输出

            with torch.no_grad():
                outputs_ter = model_t(args, **inputs)

            # loss = outputs_stu[0] / args.gradient_accumulation_steps
            loss_sum = distance_cal(args, outputs_stu, outputs_ter, **inputs)
            loss=loss_sum[0]/ args.gradient_accumulation_steps
            ...
            if step % args.gradient_accumulation_steps == 0:
                ...
                torch.nn.utils.clip_grad_norm_(model_s.parameters(), args.max_grad_norm)
            ...
            if step == len(train_dataloader) - 1:  # 训练已达最后一步
                for tag, features in benchmarks:
                    evaluate(args, model_s, optimizer, features, current_epoch, tag=tag)
...
def evaluate(args, model, optimizer, features, current_epoch, tag="dev_or_test"):
    ...
    for step, f in enumerate(features):
        ...
        tmp_eval_loss, start_logits, end_logits = logits[:3] #学生模型的start end logits
...
def main(args):
    ...
    # 模型和分词器
    tokenizer = AutoTokenizer.from_pretrained(args.model_ter_name_or_path[0])  # 分词器

    # model = Create_model_disti(args)  # 模型
    model_t = BertSpan_teacher(args, args.model_ter_name_or_path[0])
    model_s = BertSpan_student(args, args.model_stu_name_or_path[0])
    ...
    train(args, model_s, model_t,train_features, benchmarks)  # 调用train函数
    # testing(args, model, re_test_features)

if __name__ == "__main__":
    for i in range(42, 43):
        from Set_Config import set_bert_span_regularization
        ...
        output_model = os.path.join(mkpath, "model.bin")
