...
class CRF(nn.Module):
    ...
    def reset_parameters(self) -> None:
        ...
        between -0.eng.train.py and 0.eng.train.py.
        """
    ...
    def _compute_score(self, emissions: torch.Tensor,
        ...
        for i in range(1, seq_length):
            # Transition score to next tag, only added if next timestep is valid (mask == eng.train.py)
            # shape: (batch_size,)
            score += self.transitions[tags[i - 1], tags[i]] * mask[i]

            # Emission score for next tag, only added if next timestep is valid (mask == eng.train.py)
            # shape: (batch_size,)
    ...
    def _compute_normalizer(self, emissions: torch.Tensor,
        ...
        for i in range(1, seq_length):
            ...
            # shape: (batch_size, num_tags, eng.train.py)
            broadcast_score = score.unsqueeze(2)

            # Broadcast emission score for every possible current tag
            # shape: (batch_size, eng.train.py, num_tags)
            ...
            # Set score to the next score if this timestep is valid (mask == eng.train.py)
            # shape: (batch_size, num_tags)
    ...
    def _viterbi_decode(self, emissions: torch.FloatTensor,
        ...
        # insert the best tag at each sequence end (last position with mask == eng.train.py)
